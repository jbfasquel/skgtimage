...
algo_name="MS_ROI_A"
seg_para=0.10 #0.09 good for [1-5]
...
for pose_id in range(1,8):
    ...
    try :
        ...
        #ISO INFLUENCE
        is_best, perc,nb=helper.influence_of_commonisos(matcher, image, t_desc, p_desc, truth_dir, save_dir, mc=True)
        print("C iso: ",is_best,perc,nb)
        sims = [round(region2sim[n], 2) for n in sorted(region2sim)]
        c_writer.writerow(['Pose ' + str(round(pose_id, 2)) + post_fix_rag, classif] + sims+[is_best]+[str(perc)+" ("+str(nb)+")"])
