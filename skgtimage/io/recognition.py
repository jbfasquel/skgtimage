...
def save_recognizer_details(recognizer,save_dir,full=False,slices=[]):
    ...
    if recognizer.relabelled_final_t_graph is not None:
        ...
        save_graph_regions(recognizer.relabelled_final_t_graph, directory=save_dir, slices=slices)
    ...
    for step in range(0,len(recognizer.intermediate_operations)):
        ...
        if recognizer.intermediate_graphs[step] is not None:
            ...
            # Simplified
            save_graph_basic(t_graph, name="g_topological_simple_" + str(nb_nodes), directory=tmp_dir, tree=True)
            save_graph_basic(p_graph, name="g_photometric_simple_" + str(nb_nodes), directory=tmp_dir, tree=True)
            if step == (len(recognizer.intermediate_operations)-1):
                save_intensities(p_graph, directory=tmp_dir)
                save_graph_regions(t_graph, directory=tmp_dir, slices=slices)
    ...
    if recognizer.common_isomorphisms is not None:
        ...
        if recognizer.matching is not None:
            ...
            save_matching(recognizer.t_graph, recognizer.ref_t_graph, [matching_links], ['red'], name="1_matching_t", directory=tmp_dir, tree=True)
            save_matching(recognizer.p_graph, recognizer.ref_p_graph, [matching_links], ['red'], name="1_matching_p", directory=tmp_dir, tree=True)
            #Simplified
            save_matching_basic(recognizer.t_graph, recognizer.ref_t_graph, [matching_links], ['red'], name="1_matching_t_simple", directory=tmp_dir, tree=True)
            save_matching_basic(recognizer.p_graph, recognizer.ref_p_graph, [matching_links], ['red'],
        ...
        if len(recognizer.common_isomorphisms) < 20:
            for i in range(0,len(recognizer.common_isomorphisms)):
                ...
                save_matching(recognizer.t_graph, recognizer.ref_t_graph, [matching_links], ['red'],
                              name="common_iso_t_" + str(i), directory=tmp_dir, tree=True)
                save_matching(recognizer.p_graph, recognizer.ref_p_graph, [matching_links], ['red'],
                              name="common_iso_p_" + str(i), directory=tmp_dir, tree=True)
        if full:
            save_intensities(recognizer.p_graph, directory=tmp_dir)
            save_graph_regions(recognizer.t_graph, directory=tmp_dir, slices=slices)
    ...
    if (recognizer.matching is not None) and (recognizer.ordered_merges is not None):
        # All merging
        matching_links = matching2links(recognizer.matching)
        save_matching(recognizer.t_graph, recognizer.ref_t_graph, [matching_links, recognizer.ordered_merges],
                      ['red', 'green'], label_lists=[[], range(0, len(recognizer.ordered_merges) + 1)], name="matching_t", directory=tmp_dir, tree=True)
        save_matching(recognizer.p_graph, recognizer.ref_p_graph, [matching_links, recognizer.ordered_merges],
                      ['red', 'green'], label_lists=[[], range(0, len(recognizer.ordered_merges) + 1)], name="matching_p",
                      directory=tmp_dir, tree=True)
        #Simplified
        save_matching_basic(recognizer.t_graph, recognizer.ref_t_graph, [matching_links, recognizer.ordered_merges],
                            ['red', 'green'], label_lists=[[], range(0, len(recognizer.ordered_merges) + 1)],
                            name="matching_t_simplified",
                            directory=tmp_dir, tree=True)
        save_matching_basic(recognizer.p_graph, recognizer.ref_p_graph, [matching_links, recognizer.ordered_merges],
                            ['red', 'green'], label_lists=[[], range(0, len(recognizer.ordered_merges) + 1)],
